...
func TestChangeToAddress_implicitFee_matchesCeilJSFormula(t *testing.T) {
	...
	require.NoError(t, tx.ChangeToAddress("mwV3YgnowbJJB3LcyCuqiKpdivvNNFiK7M", tbc.NewFeeQuote()))

	std, err := tbc.NewFeeQuote().Fee(tbc.FeeTypeStandard)
	require.NoError(t, err)
	// 224B，默认 MAPI 5 sat/10B → feePerKb=500 → ceil(224*500/1000)=112
	wantEst := 224
	wantFee := tbc.CeilMiningFeeFromEstimatedBytes(wantEst, std.MiningFee)
...
func TestCeilMiningFeeFromEstimatedBytes_golden(t *testing.T) {
	assert.Equal(t, uint64(112), tbc.CeilMiningFeeFromEstimatedBytes(224, tbc.FeeUnit{Satoshis: 5, Bytes: 10}))
	assert.Equal(t, uint64(23), tbc.CeilMiningFeeFromEstimatedBytes(224, tbc.FeeUnit{Satoshis: 100, Bytes: 1000}))
	assert.Equal(t, uint64(1), tbc.CeilMiningFeeFromEstimatedBytes(1, tbc.FeeUnit{Satoshis: 100, Bytes: 1000}))
}
