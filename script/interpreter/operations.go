...
var opcodeArray = [256]opcode{
	...
	// New opcodes.
	script.OpPUSHMETA:    {script.OpPUSHMETA, "OP_PUSH_META", 1, opcodePushMeta},
	script.OpPARTIALHASH: {script.OpPARTIALHASH, "OP_PARTIAL_HASH", 1, opcodePartialHash},
...
func opcodeCheckSig(op *ParsedOpcode, t *thread) error {
	...
	if !t.hasFlag(scriptflag.EnableSighashForkID) || !shf.Has(sighash.ForkID) {
		...
		subScript.FindAndDelete(script.NewFromBytes([]byte{script.OpCODESEPARATOR}))
	}
...
func opcodeCheckMultiSig(op *ParsedOpcode, t *thread) error {
	...
	for _, sigInfo := range signatures {
		...
		subScr.FindAndDelete(&s)
		subScr.FindAndDelete(script.NewFromBytes([]byte{script.OpCODESEPARATOR}))
	}
	...
	for numSignatures > 0 {
		...
		// Generate the signature hash based on the signature hash type.
		txCopy := t.tx.Clone()
		txCopy.Inputs[t.inputIdx].PreviousTxScript = subScr
...
func opcodePartialHash(op *ParsedOpcode, t *thread) error {
	...
	result := partialsha256.SuffixBlockSHA256(bufPartialHash, bufData, completeSize)
	t.dstack.PushByteArray(result)
	return nil
}
