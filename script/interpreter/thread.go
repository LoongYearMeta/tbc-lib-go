...
func (t *thread) IsPushOnly() bool {
	...
	pscript, err := t.scriptParser.Parse(scr)
...
func (t *thread) apply(opts *execOpts) error {
	...
	t.scripts = make([]*script.Script, 2)

	copy(t.scripts, []*script.Script{uscript, lscript})
...
func (t *thread) Step() (bool, error) {
	...
	if t.bip16 && !t.afterGenesis && t.scriptIdx <= 2 {
		...
		case 2:
			...
			scrBytes := t.savedFirstStack[len(t.savedFirstStack)-1]
			pops := script.NewFromBytes(scrBytes)
