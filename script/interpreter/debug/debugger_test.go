...
import (
	...
	"github.com/LoongYearMeta/tbc-lib-go/script/interpreter/debug"
...
func TestDebugger_BeforeExecute(t *testing.T) {
	...
	for name, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			lscript, err := script.NewFromHexString(test.lockingScriptHex)
			assert.NoError(t, err)

			uscript, err := script.NewFromHexString(test.unlockingScriptHex)
			assert.NoError(t, err)
