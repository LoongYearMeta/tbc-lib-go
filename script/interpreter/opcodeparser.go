...
type OpcodeParser interface {
	...
	GetParsedOpcode(i *int, s *script.Script) (ParsedOpcode, error)
...
// Parse takes a *script.Script and returns a interpreter.ParsedOpcode
func (p *DefaultOpcodeParser) GetParsedOpcode(i *int, s *script.Script) (ParsedOpcode, error) {
	script := s.Bytes()
...
func (p *DefaultOpcodeParser) Unparse(pscr ParsedScript) (*script.Script, error) {
	...
	return script.NewFromBytes(buf), nil
}
