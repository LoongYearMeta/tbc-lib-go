...
func parseShortForm(src string) (*script.Script, error) {
	...
	for _, tok := range tokens {
		...
		if num, err := strconv.ParseInt(tok, 10, 64); err == nil {
			if num == 0 {
				scr = append(scr, script.Op0)
			} else if num == -1 || (1 <= num && num <= 16) {
				scr = append(scr, (script.Op1-1)+byte(num))
			} else {
				n := &scriptNumber{val: big.NewInt(num)}
				tmpScr := script.NewFromBytes(scr)
				if err := tmpScr.AppendPushData(n.Bytes()); err != nil {
					return nil, err
				}
				scr = tmpScr.Bytes()
			}
			...
			tok[0] == '\'' && tok[len(tok)-1] == '\'' {
			tmpScr := script.NewFromBytes(scr)
			if err := tmpScr.AppendPushData([]byte(tok[1 : len(tok)-1])); err != nil {
				return nil, err
			}
			scr = tmpScr.Bytes()
	...
	return script.NewFromBytes(scr), nil
}
