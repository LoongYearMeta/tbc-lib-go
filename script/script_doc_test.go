...
func TestScriptCreation_P2PKH(t *testing.T) {
	...
	script, err := script.NewP2PKHFromAddress(address)
...
func TestScriptCreation_P2MS(t *testing.T) {
	...
	threshold := 2
	script, err := script.BuildMultisigOut(pubKeys, threshold, nil)
	assert.NoError(t, err)
	assert.NotNil(t, script)
...
func TestScriptCreation_P2SH(t *testing.T) {
	...
	redeemScript, err := script.BuildMultisigOut(pubKeys, 2, nil)
	assert.NoError(t, err)
...
func TestScriptCreation_DataOutput(t *testing.T) {
	...
	// 创建一个数据输出脚本
	data := "hello world!!!"
	dataScr, err := script.BuildDataOut([]byte(data), "")
	assert.NoError(t, err)
	assert.NotNil(t, dataScr)

	// 验证脚本结构
	assert.True(t, dataScr.IsData())
	assert.True(t, dataScr.IsDataOut())

	// 获取 ASM 表示
	asm, err := dataScr.ToASM()
	assert.NoError(t, err)
	assert.Contains(t, asm, "OP_RETURN")

	// 测试安全数据输出（OP_FALSE OP_RETURN ...）
	safeScript, err := script.BuildSafeDataOut([]byte(data), "")
	assert.NoError(t, err)
...
func TestScriptCreation_CustomScripts(t *testing.T) {
	...
	// 创建一个自定义脚本
	scr := script.NewFromBytes([]byte{})

	// 添加操作码
	err := scr.Add(script.OpIF)
	assert.NoError(t, err)

	// 前置操作码
	err = scr.Prepend(script.Op2SWAP)
	assert.NoError(t, err)

	// 添加另一个操作码
	err = scr.Add(script.OpNOT)
	assert.NoError(t, err)
...
func TestScriptParsingAndIdentification(t *testing.T) {
	...
	scr := script.NewFromBytes(rawScript)
	assert.NotNil(t, scr)
	...
	// 验证脚本类型识别
	assert.False(t, scr.IsPublicKeyHashOut())
	assert.False(t, scr.IsScriptHashOut())
	assert.True(t, scr.IsMultisigOut())
	assert.True(t, scr.IsMultiSigOut())

	// 测试分类
	scriptType := scr.Classify()
	assert.Equal(t, script.ScriptTypeMultisigOut, scriptType)
}
...
func TestScriptFromString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			script, err := script.FromString(tt.str)
			if tt.str == "" {
				// 空字符串应该创建空脚本
				assert.NoError(t, err)
				assert.NotNil(t, script)
...
func TestScriptFromChunks(t *testing.T) {
	...
	// 手动创建 chunks
	chunks := []script.Chunk{
		{OpcodeNum: script.OpDUP},
		{OpcodeNum: script.OpHASH160},
		{OpcodeNum: script.OpDATA20, Buf: make([]byte, 20), Len: 20},
		{OpcodeNum: script.OpEQUALVERIFY},
		{OpcodeNum: script.OpCHECKSIG},
	}

	script, err := script.FromChunks(chunks)
	assert.NoError(t, err)
	assert.NotNil(t, script)
...
func TestScriptChunks(t *testing.T) {
	...
	// 创建一个 P2PKH 脚本
	scr, err := script.NewFromHexString("76a914e2a623699e81b291c0327f408fea765d534baa2a88ac")
	assert.NoError(t, err)
	...
	assert.Equal(t, script.OpDUP, chunks[0].OpcodeNum)

	// 验证第二个 chunk 是 OP_HASH160
	assert.Equal(t, script.OpHASH160, chunks[1].OpcodeNum)

	// 验证第三个 chunk 包含数据
	assert.Equal(t, script.OpDATA20, chunks[2].OpcodeNum)
...
func TestScriptClone(t *testing.T) {
	...
	original, err := script.NewFromHexString("76a914e2a623699e81b291c0327f408fea765d534baa2a88ac")
	assert.NoError(t, err)
...
func TestScriptRemoveCodeseparators(t *testing.T) {
	...
	scr := script.NewFromBytes([]byte{})
	_ = scr.AppendOpcodes(script.OpDUP, script.OpCODESEPARATOR, script.OpHASH160)

	// 移除 codeseparators
	scr.RemoveCodeseparators()
...
func TestScriptSubScript(t *testing.T) {
	...
	// 创建一个包含 OP_CODESEPARATOR 的脚本
	scr := script.NewFromBytes([]byte{})
	_ = scr.AppendOpcodes(script.OpDUP, script.OpCODESEPARATOR, script.OpHASH160, script.OpEQUALVERIFY)

	// 从第一个 codeseparator 开始获取子脚本
	subScript := scr.SubScript(0)
...
func TestScriptCheckMinimalPush(t *testing.T) {
	...
	for i := range chunks {
		if chunks[i].Buf != nil {
			// 这是一个数据推送，验证它是最小的
			isMinimal := scr.CheckMinimalPush(i)
...
func TestScriptIsPushOnly(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			script, err := script.NewFromHexString(tt.script)
			assert.NoError(t, err)
...
func TestScriptGetData(t *testing.T) {
	...
	// 测试 P2PKH 脚本
	p2pkhScript, err := script.NewFromHexString("76a914e2a623699e81b291c0327f408fea765d534baa2a88ac")
	assert.NoError(t, err)
	...
	// 测试数据输出脚本
	dataScript, err := script.BuildDataOut([]byte("hello"), "")
	assert.NoError(t, err)
...
func TestScriptGetPublicKeyHash(t *testing.T) {
	...
	script, err := script.NewP2PKHFromPubKeyHash(pubKeyHash)
	assert.NoError(t, err)
...
func TestScriptGetSignatureOperationsCount(t *testing.T) {
	...
	count := scr.GetSignatureOperationsCount(true)
	...
	multisigScript, err := script.BuildMultisigOut(pubKeys, 2, nil)
	assert.NoError(t, err)
...
func TestScriptClassify(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expected: script.ScriptTypeMultisigOut,
			isOutput: true,
		},
		{
			name:     "data output",
			script:   "6a0e68656c6c6f20776f726c64212121",
			expected: script.ScriptTypeDataOut,
			isOutput: true,
		},
	}
...
func TestScriptToAddress(t *testing.T) {
	...
	script, err := script.NewP2PKHFromAddress(addressStr)
	assert.NoError(t, err)
...
func TestScriptFindAndDelete(t *testing.T) {
	...
	scriptToDelete := script.NewFromBytes([]byte{})
	_ = scriptToDelete.AppendPushData(data)

	// 查找并删除
	count := scr.FindAndDelete(scriptToDelete)
