...
func TestDecodeParts(t *testing.T) {
	...
	t.Run("simple", func(t *testing.T) {
		parts, err := script.DecodeStringParts("05000102030401FF02ABCD")
	...
	t.Run("empty parts", func(t *testing.T) {
		parts, err := script.DecodeStringParts("")
		assert.NoError(t, err)
	...
	t.Run("bad parts", func(t *testing.T) {
		_, err := script.DecodeStringParts("05000000")
		assert.Error(t, err)
		assert.EqualError(t, err, "not enough data")

		_, err = script.DecodeStringParts("4c05000000")
		assert.Error(t, err)
		assert.EqualError(t, err, "not enough data")
	})
	...
	t.Run("decode using OP_PUSHDATA1", func(t *testing.T) {
		...
		decoded, err := script.DecodeParts(b)
		assert.NoError(t, err)
