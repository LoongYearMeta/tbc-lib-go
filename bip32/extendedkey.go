...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	if ilNum.Cmp(bec.S256().N) >= 0 || ilNum.Sign() == 0 {
	...
	if k.isPrivate {
		...
		ilx, ily := bec.S256().ScalarBaseMult(il)
		...
		//
		// childKey = serP(point(parse256(Il)) + parentKey)
		childX, childY := bec.S256().Add(ilx, ily, pubKey.X, pubKey.Y)
		pk := bec.PublicKey{Curve: bec.S256(), X: childX, Y: childY}
...
func (k *ExtendedKey) ECPubKey() (*bec.PublicKey, error) {
	return bec.ParsePubKey(k.pubKeyBytes(), bec.S256())
}
...
func (k *ExtendedKey) ECPrivKey() (*bec.PrivateKey, error) {
	if !k.isPrivate {
...
func NewMaster(seed []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	...
	if secretKeyNum.Cmp(bec.S256().N) >= 0 || secretKeyNum.Sign() == 0 {
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	if isPrivate {
		...
		if keyNum.Cmp(bec.S256().N) >= 0 || keyNum.Sign() == 0 {
			return nil, ErrUnusableSeed
		}
	} else {
