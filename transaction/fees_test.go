...
func TestExtractDataFee(t *testing.T) {
	t.Run("get valid data fee", func(t *testing.T) {
		...
		assert.Equal(t, 100, fee.MiningFee.Satoshis)
		assert.Equal(t, 1000, fee.MiningFee.Bytes)
		assert.Equal(t, 100, fee.RelayFee.Satoshis)
		assert.Equal(t, 1000, fee.RelayFee.Bytes)
	})
